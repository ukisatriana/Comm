...
const GeneralField = ({ handleFileUpload }) => {
  ...
  const handleUploadChange = async ({ file }) => {
    ...
    const storageRef = ref(storage, `images/subcon/${file.name}`);
    ...
    <Row gutter={16}>
      <Col xs={24} sm={24} md={17}>
        <Card title="Subcontractor Info">
          <Form.Item name="subcontractorName" label="Subcontractor Name">
            <Input placeholder="Input Contractor Name" />
          </Form.Item>
          <Form.Item name="contractorAddress" label="Subcontractor Address">
            <Input.TextArea
              placeholder="Input Subcontractor address"
              rows={4}
            />
          </Form.Item>
        </Card>
