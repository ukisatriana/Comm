...
const ProcurmentForm = ({ mode = 'ADD', param }) => {
	...
	useEffect(() => {
		if (mode === 'EDIT' && param?.id) {
		  const fetchDocument = async () => {
			const docRef = doc(db, 'procurmentDocs', param.id);
			...
			if (docSnap.exists()) {
			  ...
			  navigate('/app/apps/documents/procurment/procurment-list');
			}
		  };
		  fetchDocument();
		}
	  ...
	  const onFinish = async (values) => {
		...
		try {
		  ...
		  if (mode === 'ADD') {
			await addDoc(collection(db, 'procurmentDocs'), {
			  ...docData,
			  createdAt: new Date(),
			});
	...
	return (
		<>
			<Form
				layout="vertical"
				form={form}
				initialValues={{ projectName: '', procurmentDocsName: '' }}
				onFinish={onFinish}
			>
				<PageHeaderAlt className="border-bottom" overlap>
					<div className="container">
						<Flex className="py-2" mobileFlex={false} justifyContent="space-between" alignItems="center">
							<h2 className="mb-3">{mode === 'ADD'? 'Add New Vendor Data & Docs' : `Edit Vendor Data & Docs`} </h2>
							<div className="mb-3">
