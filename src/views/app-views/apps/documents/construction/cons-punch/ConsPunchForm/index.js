...
const ConsPunchForm = ({ mode = 'ADD', param }) => {
	...
	useEffect(() => {
		if (mode === 'EDIT' && param?.id) {
		  const fetchDocument = async () => {
			const docRef = doc(db, 'consPunchList', param.id);
			...
			if (docSnap.exists()) {
			  ...
			  navigate('/app/apps/documents/construction/cons-punch/cons-punch-list');
			}
		  };
		  fetchDocument();
		}
	  ...
	  const onFinish = async (values) => {
		...
		try {
		  ...
		  if (mode === 'ADD') {
			await addDoc(collection(db, 'consPunchList'), {
			  ...docData,
			  createdAt: new Date(),
			});
	...
	return (
		<>
			<Form
				layout="vertical"
				form={form}
				initialValues={{ projectName: '', consPunchDocsName: '' }}
				onFinish={onFinish}
			>
				<PageHeaderAlt className="border-bottom" overlap>
					<div className="container">
						<Flex className="py-2" mobileFlex={false} justifyContent="space-between" alignItems="center">
							<h2 className="mb-3">{mode === 'ADD'? 'Add New Construction Punch' : `Edit Construction Punch`} </h2>
							<div className="mb-3">
