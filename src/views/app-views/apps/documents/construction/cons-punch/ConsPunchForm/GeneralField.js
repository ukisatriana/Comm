...
const GeneralField = ({ handleFileUpload }) => {
	const [uploadLoading, setUploadLoading] = useState(false);
	 ...
	 const handleUploadChange = async ({ file }) => {
		...
		const storageRef = ref(storage, `documents/construction/consPunclistDocs/${file.name}`);
	...
	<Row gutter={16}>
		<Col xs={24} sm={24} md={17}>
			<Card title="Basic Info">
				<Form.Item name="consPunchNameDocs" label="Construction Punch Name" >
					<Input placeholder="Construction Punch Name" />
				</Form.Item>
