...
const QcTestReportForm = ({ mode = 'ADD', param }) => {
	  ...
	  useEffect(() => {
		if (mode === 'EDIT' && param?.id) {
		  const fetchDocument = async () => {
			const docRef = doc(db, 'qcTestReport', param.id);
			...
			if (docSnap.exists()) {
			  ...
			  navigate('/app/apps/documents/construction/qc-test-report/qc-test-report-list');
			}
		  };
		  fetchDocument();
		}
		...
		const onFinish = async (values) => {
			...
			if (mode === 'ADD') {
				await addDoc(collection(db, 'qcTestReport'), {
				  ...docData,
				  createdAt: new Date(),
				});
	...
	return (
		<>
			<Form
				layout="vertical"
				form={form}
				initialValues={{ projectName: '', qcTestReportDocsName: '' }}
				onFinish={onFinish}
			>
				<PageHeaderAlt className="border-bottom" overlap>
					<div className="container">
						<Flex className="py-2" mobileFlex={false} justifyContent="space-between" alignItems="center">
							<h2 className="mb-3">{mode === 'ADD'? 'Add New Qc Test Report' : `Edit Qc Test Report`} </h2>
							<div className="mb-3">
...
export default QcTestReportForm
