...
const GeneralField = ({ handleFileUpload }) => {
	const [uploadLoading, setUploadLoading] = useState(false);
	  ...
	  const handleUploadChange = async ({ file }) => {
		...
		const storageRef = ref(storage, `documents/construction/consTestRepDocs${file.name}`);
	...
	<Row gutter={16}>
		<Col xs={24} sm={24} md={17}>
			<Card title="Basic Info">
				<Form.Item name="constTestReportName" label="Construction Test Report Name" >
					<Input placeholder="Cons Test Report Name" />
				</Form.Item>
