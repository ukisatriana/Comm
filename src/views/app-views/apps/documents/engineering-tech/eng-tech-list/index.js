...
const EngTechList = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const documents = await FirestoreService.getDocuments('engineeringTech');
				setList(documents);
			} catch (error) {
		...
		Modal.confirm({
		  ...
		  onOk: async () => {
			try {
			  ...
			  await deleteDoc(doc(db, 'engineeringTech', row.id));
	...
	const tableColumns = [
		...
		},
		{
			...
			dataIndex: 'commisioningDocsName',
			sorter: (a, b) => utils.antdTableSorter(a, b, 'commisioningDocsName')
		},
		{
			title: 'Project Name',
